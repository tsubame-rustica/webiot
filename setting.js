...
//かたさを変更するスライドバー設定
function setkatasa(event){
	var katasa=event.target.value;
    //console.log(angle);
	//channel.send({slope:angle});
	messageDiv.innerText="かたさ:"+katasa+"に設定しました";
}

function showkatasa(event){
	katasaGuide.innerText=event.target.value;
}

//モータドライバ用設定
function katasapwm(ratio) {
  //var pca9685pwm = await pca9685pwmPromise;
  //await pca9685pwm.setPWM(0, ratio);
  katasaDiv.innerText="かたさ:"+ratio*10+"に設定しました";
}
